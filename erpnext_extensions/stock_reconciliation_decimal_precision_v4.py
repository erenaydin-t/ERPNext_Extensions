...
# License: MIT
"""Stock Reconciliation DECIMAL(30,9) allowlist + migrate-safe helpers (v4).

Covers only Stock Reconciliation and Stock Reconciliation Item with an explicit
static allowlist. Does not scan DocType fields at runtime or widen unrelated stock DocTypes.
"""
...
STOCK_RECONCILIATION_FIELDS_BY_DOCTYPE: dict[str, tuple[str, ...]] = {
...
class StockReconciliationFieldTarget:
	doctype: str
	fieldname: str
...
def stock_reconciliation_field_targets() -> tuple[StockReconciliationFieldTarget, ...]:
	return tuple(
		StockReconciliationFieldTarget(doctype=doctype, fieldname=fieldname)
		for doctype, fieldnames in STOCK_RECONCILIATION_FIELDS_BY_DOCTYPE.items()
...
def verify_and_set_metadata(logger) -> list[dict[str, Any]]:
	...
	for target in stock_reconciliation_field_targets():
		row = {
	...
	logger.info(
		"Stock Reconciliation metadata summary: total=%s changed=%s skipped=%s errors=%s actions=%s",
		summary["total"],
		len(summary["changed"]),
		len(summary["skipped"]),
		len(summary["errors"]),
...
def apply_decimal_schema_targets(logger) -> list[dict[str, Any]]:
	...
	logger.info(
		"Stock Reconciliation schema summary: total=%s changed=%s skipped=%s errors=%s actions=%s",
		summary["total"],
		len(summary["changed"]),
		len(summary["skipped"]),
		len(summary["errors"]),
