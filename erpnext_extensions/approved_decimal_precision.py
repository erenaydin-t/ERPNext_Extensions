...
APPROVED_FIELDS_BY_DOCTYPE: dict[str, tuple[str, ...]] = {
	"Facility": (
		"principal_amount",
		"profit_amount",
	...
	"Sales Order Item": ("amount", "base_amount", "net_amount", "base_net_amount"),
	"Sales Invoice Item": ("amount", "base_amount", "net_amount", "base_net_amount"),
	"Sales Taxes and Charges": ("tax_amount", "base_tax_amount", "total", "base_total"),
}


@dataclass(frozen=True)
class ApprovedFieldTarget:
	doctype: str
	fieldname: str
...
def approved_field_targets() -> tuple[ApprovedFieldTarget, ...]:
	return tuple(
		ApprovedFieldTarget(doctype=doctype, fieldname=fieldname)
		for doctype, fieldnames in APPROVED_FIELDS_BY_DOCTYPE.items()
		for fieldname in fieldnames
	)


def approved_tables_and_columns() -> dict[str, tuple[str, ...]]:
	return {f"tab{doctype}": tuple(fieldnames) for doctype, fieldnames in APPROVED_FIELDS_BY_DOCTYPE.items()}
...
def verify_and_set_metadata(logger) -> list[dict[str, Any]]:
	...
	for target in approved_field_targets():
		row = {
		...
		try:
			...
			action, final_length = ensure_length_property_setter(
				target.doctype, target.fieldname, target_length, logger
			)
