...
TABLES_AND_COLUMNS = {
	# Parent
	"tabJournal Entry": ["total_debit", "total_credit", "difference", "total_amount"],
	# Child
	...
	# Payment Entry
	"tabPayment Entry": ["paid_from_account_balance", "paid_to_account_balance", "party_balance"],
...
def execute():
	...
	current_length_row = db.sql(
        ...
        WHERE TABLE_SCHEMA=%s AND TABLE_NAME=%s AND COLUMN_NAME=%s
    """,
		(db_name, text_field_table, text_field_column),
		as_dict=True,
	)
	...
	try:
		...
		frappe.logger().warning(f"Could not update doctype field definition: {str(e)}")
