...
# License: MIT
"""Set Stock Reconciliation amount/rate/qty field metadata so migrate sync keeps DECIMAL(30,9).
...
from erpnext_extensions.stock_reconciliation_precision import (
	TARGET_LENGTH,
	numeric_fields_for_doctype,
	stock_reconciliation_doctypes,
)
...
def execute() -> None:
	logger = frappe.logger("erpnext_extensions.set_stock_reconciliation_amount_decimal_metadata")
	logger.info("Starting set_stock_reconciliation_amount_decimal_metadata")

	for doctype in stock_reconciliation_doctypes():
		meta = frappe.get_meta(doctype, cached=False)
		known = {df.fieldname for df in meta.fields}
		for fieldname in numeric_fields_for_doctype(doctype):
	...
	logger.info("Completed set_stock_reconciliation_amount_decimal_metadata")
	frappe.db.commit()
