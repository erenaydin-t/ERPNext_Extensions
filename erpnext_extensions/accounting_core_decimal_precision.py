...
# License: MIT
"""Accounting Core DECIMAL(30,9) allowlist + migrate-safe metadata/schema helpers.

Covers Journal Entry, Journal Entry Account, GL Entry, and Account Closing Balance.
Does not widen Payment Entry fields.
"""
...
ACCOUNTING_CORE_FIELDS_BY_DOCTYPE: dict[str, tuple[str, ...]] = {
	"Journal Entry": (
		"total_debit",
		"total_credit",
		"difference",
...
class AccountingCoreFieldTarget:
	doctype: str
	fieldname: str
...
def accounting_core_field_targets() -> tuple[AccountingCoreFieldTarget, ...]:
	return tuple(
		AccountingCoreFieldTarget(doctype=doctype, fieldname=fieldname)
		for doctype, fieldnames in ACCOUNTING_CORE_FIELDS_BY_DOCTYPE.items()
		for fieldname in fieldnames
	)


def accounting_core_tables_and_columns() -> dict[str, tuple[str, ...]]:
	return {
		f"tab{doctype}": tuple(fieldnames)
		for doctype, fieldnames in ACCOUNTING_CORE_FIELDS_BY_DOCTYPE.items()
	}
...
def verify_and_set_metadata(logger) -> list[dict[str, Any]]:
	...
	for target in accounting_core_field_targets():
		row = {
