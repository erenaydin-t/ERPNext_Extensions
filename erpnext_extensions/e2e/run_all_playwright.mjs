...
function parseOutcome(text) {
  const m =
    text.match(/"all_ok"\s*:\s*(true|false)/) ||
    text.match(/"pass"\s*:\s*(true|false)/);
...
function runScript(absPath, { retries }) {
  ...
  for (
    let r = 0;
    r < retries && last.exit_code !== 0 && last.all_ok !== true;
    r++
  ) {
...
async function runParallel(batch, jobs, retries) {
  ...
  const workers = Array.from({ length: Math.min(jobs, batch.length) }, () =>
    worker()
  );
...
const passed = rows.filter(
  (r) => r.exit_code === 0 && r.all_ok !== false
).length;
const summary = {
  total: rows.length,
  passed,
