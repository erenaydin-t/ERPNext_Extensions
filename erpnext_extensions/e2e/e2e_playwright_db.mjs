...
export const BENCH = process.env.FRAPPE_BENCH_ROOT || "/workspace/development/frappe-bench";
export const SITE = process.env.FRAPPE_E2E_SITE || "development.localhost";

const GET_STATE_METHOD = "erpnext_extensions.e2e.e2e_document_state.e2e_get_document_state";
const EXISTS_METHOD = "erpnext_extensions.e2e.e2e_document_state.e2e_document_exists";

const RETRYABLE_INFRA = /deadlock|QueryDeadlockError|1020|1213|Lock wait timeout|tabSeries|OperationalError/i;
...
export function benchExecute(method, kwargs = null, args = null, { retries = 3 } = {}) {
	let lastErr;
...
export function waitWorkflowState(doctype, name, workflowState, opts = {}) {
	return waitDocumentState(doctype, name, { workflow_state: workflowState }, opts);
}
...
export async function waitDocumentAbsent(doctype, name, { timeoutMs = 90000, pollMs = 500 } = {}) {
	return waitDocumentState(doctype, name, { exists: false }, { timeoutMs, pollMs, fields: ["name"] });
}
...
export async function assertDbState({
	...
	const waitMeta = await waitDocumentState(doctype, name, expected, { timeoutMs, fields });
