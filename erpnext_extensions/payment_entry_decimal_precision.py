...
# License: MIT
"""Payment Entry DECIMAL(30,9) allowlist + migrate-safe helpers.

Isolated storage-capacity hardening for Payment Entry and monetary child tables.
Does not change Frappe global Currency/Float mapping or PE accounting behavior.
Does not scan DocType fields at runtime.
"""
...
def payment_entry_field_targets() -> tuple[PaymentEntryFieldTarget, ...]:
	return tuple(
		PaymentEntryFieldTarget(doctype=doctype, fieldname=fieldname)
		for doctype, fieldnames in PAYMENT_ENTRY_FIELDS_BY_DOCTYPE.items()
		for fieldname in fieldnames
	)


def summarize_results(results: list[dict[str, Any]], *, action_key: str = "action") -> dict[str, Any]:
	"""Collect changed / skipped / errors for patch logging."""
	changed: list[str] = []
	skipped: list[str] = []
	errors: list[str] = []
...
def verify_and_set_metadata(logger) -> list[dict[str, Any]]:
	...
	for target in payment_entry_field_targets():
		row = {
	...
	logger.info(
		"Payment Entry metadata summary: total=%s changed=%s skipped=%s errors=%s actions=%s",
		summary["total"],
		len(summary["changed"]),
		len(summary["skipped"]),
		len(summary["errors"]),
...
def apply_decimal_schema_targets(logger) -> list[dict[str, Any]]:
	...
	logger.info(
		"Payment Entry schema summary: total=%s changed=%s skipped=%s errors=%s actions=%s",
		summary["total"],
		len(summary["changed"]),
		len(summary["skipped"]),
		len(summary["errors"]),
...
def assert_schema_targets(logger=None) -> list[dict[str, Any]]:
	"""Schema guard: every allowlisted column must be DECIMAL(30,9) or wider scale=9."""
	logger = logger or frappe.logger("erpnext_extensions.payment_entry_decimal_precision")
	...
	if failures:
		detail = ", ".join(f"{r['table']}.{r['field']}={r['column_type']}/{r['action']}" for r in failures)
		raise RuntimeError(f"Payment Entry DECIMAL(30,9) schema drift detected: {detail}")
	logger.info("Payment Entry schema guard OK for %s fields", len(list(payment_entry_field_targets())))
	return failures
