...
async function countFormMessagesContaining(page, text) {
	return page.evaluate((needle) => {
		const blocks = document.querySelectorAll(".form-message-container .form-message, .form-message");
		let count = 0;
		blocks.forEach((el) => {
...
async function sectionExpandedOnLoad(page, fieldname) {
	return page.evaluate((fn) => {
		const section = document.querySelector(`.form-section[data-fieldname="${fn}"]`);
...
async function toggleSectionCollapse(page, sectionFieldname) {
	await page.locator(`.form-section[data-fieldname="${sectionFieldname}"] .section-head.collapsible`).click();
}
...
async function testSectionToggle(page, sectionFieldname, innerFieldname) {
	...
	if (!state.collapsible) {
		return { section: sectionFieldname, pass: false, reason: "not collapsible" };
	}
	await toggleSectionCollapse(page, sectionFieldname);
	await page.waitForFunction(
		(fn) => document.querySelector(`.form-section[data-fieldname="${fn}"] .section-body`)?.classList.contains("hide"),
		sectionFieldname,
		{ timeout: 15000 }
	);
	await toggleSectionCollapse(page, sectionFieldname);
	await page.waitForFunction(
		({ sectionFn, fieldFn }) => {
			const body = document.querySelector(`.form-section[data-fieldname="${sectionFn}"] .section-body`);
			return body && !body.classList.contains("hide") && !!document.querySelector(`[data-fieldname="${fieldFn}"]`);
		},
...
async function run() {
	const funded = bench("erpnext_extensions.petty_management.e2e.pm_multi_pe_prep.prepare_partial_funded_for_close_ui");
	const draft = bench("erpnext_extensions.petty_management.e2e.pm_request_ui_prep.get_draft_pm_request");
	...
	await login(page, process.env.FRAPPE_E2E_USER || "Administrator", process.env.FRAPPE_E2E_PASSWORD || "admin");

	const results = {};

	await openPmRequest(page, draft.pm_request);
	results.draft_intro_count = await countFormMessagesContaining(page, "Submit the PM Request first");
	...
	results.sections_expanded_on_load = {
		...
		payment_entries: await sectionExpandedOnLoad(page, "section_payment_entries"),
	...
	results.view_pe_before_close = await viewPaymentEntriesActionVisible(page, funded.pm_request).catch(() => false);

	const sectionTests = [];
	sectionTests.push(await testSectionToggle(page, "section_payment_entries", "payment_entries_html"));
	...
	console.log(JSON.stringify({ pass, funded: funded.pm_request, draft: draft.pm_request, results, screenshot }, null, 2));
	await browser.close();
