...
async function run() {
  ...
  try {
    ...
    await page.goto(
      `${BASE}/app/pm-request/${encodeURIComponent(partial.pm_request)}`,
      {
        waitUntil: "domcontentloaded",
      }
    );
    await waitPmRequestForm(page);
    evidence.screenshots.before_close_partial = await shot(
      page,
      "01_partial_before_close"
    );
    ...
    const duplicateActions = await page.evaluate(() => {
      ...
      const std = Array.from(
        document.querySelectorAll(".actions-btn-group")
    ...
    await page.goto(
      `${BASE}/app/pm-request/${encodeURIComponent(full.pm_request)}`,
      {
        waitUntil: "domcontentloaded",
      }
    );
    ...
    await page.goto(
      `${BASE}/app/pm-request/${encodeURIComponent(draftCase.pm_request)}`,
      {
        waitUntil: "domcontentloaded",
      }
    );
    ...
    results.push({
      ...
      pass:
        flagsDraft.can_close_pm_request === false &&
        /draft payment entries exist/i.test(
    ...
    await waitPmRequestForm(page);
    const peListVisible = await page
      .locator("#pm-request-pe-list table")
      .count();
    results.push({
    ...
    await page.goto(
      `${BASE}/app/pm-request/${encodeURIComponent(multi.pm_request)}`,
      {
        waitUntil: "domcontentloaded",
      }
    );
    ...
    results.push({
      ...
      pass:
        flagsMulti.can_reject === false &&
        flagsMulti.submitted_payment_entry_count >= 2,
      flags: flagsMulti,
    });
    results.push({
    ...
    const menuInfo = await page.evaluate(async () => {
      ...
      const items = Array.from(group.querySelectorAll(".dropdown-item")).map(
    ...
    if (viewPeVisible || menuInfo.ok) {
      await page.evaluate(
        (args) => {
          frappe.route_options = args.filters || {
            reference_no: args.pm_request,
          };
          frappe.set_route("List", "Payment Entry");
        },
        {
          filters: flagsMulti.payment_entry_list_filters,
          pm_request: multi.pm_request,
        }
      );
      await page.waitForFunction(
        () => {
      ...
      evidence.screenshots.pe_list_view = await shot(
        page,
        "06_payment_entry_list"
      );
    ...
    console.error(err);
    console.log(
      JSON.stringify({ pass: false, error: String(err), evidence }, null, 2)
    );
    process.exit(1);
  } finally {
    await browser.close();
