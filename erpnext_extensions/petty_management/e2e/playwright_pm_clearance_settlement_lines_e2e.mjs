...
const SCREEN = path.join(
  __dirname,
  "screenshots",
  "pm_clearance_settlement_lines_e2e"
);
const TRACE = path.join(
  __dirname,
  "traces",
  "pm_clearance_settlement_lines_e2e.zip"
);
const BASE =
...
function bench(method) {
  const out = execSync(
    `cd ${BENCH} && bench --site development.localhost execute "${method}"`,
    {
      encoding: "utf8",
    }
  );
...
async function setFrmValue(page, fieldname, value) {
  ...
  await page.waitForFunction(
    ({ fn, v }) =>
      (window.cur_frm?.doc?.[fn] || "").toString() === v.toString(),
    { fn: fieldname, v: value },
    { timeout: 60000 }
  );
}
...
async function prepareEmptyGridRow(page, tableFieldname) {
  ...
  if (needsNew) {
    await page
      .locator(`[data-fieldname="${tableFieldname}"] .grid-add-row`)
      .first()
      .click({ timeout: 60000 });
...
async function pruneEmptyChildRows(page) {
  await page.evaluate(() => {
    ...
    prune(
      "details",
      (r) =>
        !(r.purchase_invoice || r.purchase_order || r.supplier_advance_account)
    );
    prune(
      "request_allocations",
      (r) =>
        !(r.pm_request || r.pm_opening_advance) &&
...
async function openLastRowEditor(page, tableFieldname) {
  ...
  const grid = page
    .locator(`[data-fieldname="${tableFieldname}"] .grid-field`)
    .first();
  await grid.click({ timeout: 60000 });
  const lastRow = page
    .locator(`[data-fieldname="${tableFieldname}"] .grid-row`)
    .last();
...
async function rowEditorRoot(page, tableFieldname) {
  ...
  const scoped = page
    .locator(`[data-fieldname="${tableFieldname}"] .grid-row-open`)
    .first();
...
async function setEditableGridLinkBySearch(
  page,
  tableFieldname,
  linkFieldname,
  searchText
  ...
  let input = row
    .locator(`.frappe-control[data-fieldname="${linkFieldname}"] input`)
    .first();
...
async function setGridCellLinkBySearch(
  page,
  tableFieldname,
  linkFieldname,
  searchText
  ...
  await page.waitForFunction(
    ({ fn }) => {
      const roots = Array.from(
        document.querySelectorAll(".grid-row-open, .modal-dialog")
      );
      const root = roots.find((r) => r && r.offsetParent) || document;
      const control = root.querySelector(
        `.frappe-control[data-fieldname="${fn}"]`
      );
  ...
  await page.evaluate(
    ({ fn }) => {
      ...
      const field =
        control &&
        (control.__control ||
          window.cur_frm?.cur_grid?.grid_form?.fields_dict?.[fn]);
      const ul = field?.awesomplete?.ul;
      const options = Array.from(ul?.querySelectorAll('[role="option"]') || []);
      const pick =
        options.find((el) => {
...
async function saveForm(page) {
  ...
  try {
    ...
    const snap = await page.evaluate(() => {
      const dlg = document.querySelector(
        ".msgprint-dialog .modal-body, .msgprint"
      );
      return {
  ...
  await page.waitForFunction(() => !window.cur_frm?.is_loading, {
    timeout: 180000,
  });
}
...
async function applyWorkflowAction(page, actionLabel) {
  const btn = page
    .locator(".page-actions .btn, .standard-actions .btn")
...
async function approveClearanceForSettlement(page) {
  ...
  await page.evaluate(async (name) => {
    ...
    if (r.exc) {
      throw new Error(
        typeof r.exc === "string" ? r.exc : JSON.stringify(r.exc)
      );
    }
...
async function settlePettyCash(page) {
  ...
  const je =
    ...
    (await page.evaluate(() => window.cur_frm?.doc?.journal_entry || ""));
  if (!je) {
    throw new Error(
      `journal_entry not set after settle: ${JSON.stringify(result)}`
    );
  }
  return je;
}
...
async function openLinkedJE(page, jeName) {
  ...
  await page.goto(`${BASE}/app/journal-entry/${encodeURIComponent(je)}`, {
    waitUntil: "domcontentloaded",
    timeout: 180000,
  });
...
async function holderSnapshot(page, prep) {
  return page.evaluate(async (p) => {
    const r = await frappe.call({
      method:
        "erpnext_extensions.petty_management.doctype.pm_clearance.pm_clearance.get_pm_clearance_holder_context",
      args: {
        employee: p.employee,
        company: p.company,
...
async function run() {
  ...
  const evidence = {
    screenshots: {},
    trace: TRACE,
    logs: { consoleErrors: [], requestFailures: [] },
  };
  const results = [];
  ...
  try {
    ...
    await setEditableGridLinkBySearch(
      page,
      "details",
      "purchase_invoice",
      prep.supplier_name
    );
    evidence.screenshots.pi_dropdown_selected = await shot(
      page,
      "03_pi_dropdown_selected"
    );
    ...
    const currencyShown = await page.evaluate(() =>
      (window.cur_frm?.doc?.currency || "").toString()
    );
    results.push({
      test: "pi_currency",
      pass: currencyShown === prep.currency,
      currencyShown,
    });
    ...
    const detailAlloc = await page.evaluate(() => {
      const rows = window.cur_frm?.doc?.details || [];
      return rows.length
        ? Number(rows[rows.length - 1].allocated_amount || 0)
        : 0;
    });
    await page.evaluate(
      async ({ req, amt }) => {
    ...
    await saveForm(page);
    const clearanceNamePi = await page.evaluate(
      () => window.cur_frm?.doc?.name || ""
    );
    results.push({
      test: "pi_saved_has_name",
      pass: !!clearanceNamePi,
      clearanceNamePi,
    });

    // Reload doc and verify persistence
    await page.goto(
      `${BASE}/app/pm-clearance/${encodeURIComponent(clearanceNamePi)}`,
      {
        waitUntil: "domcontentloaded",
      }
    );
    ...
    results.push({
      test: "pi_persisted_name",
      pass:
        (await page.evaluate(() => window.cur_frm?.doc?.name)) ===
        clearanceNamePi,
    });
    ...
    evidence.screenshots.pi_preview = await shot(
      page,
      "08_pi_preview_modal_closed"
    );
    ...
    evidence.screenshots.po_dropdown_selected = await shot(
      page,
      "15_po_dropdown_selected"
    );
    await page.evaluate(
      ({ account, amt }) => {
        const grid = window.cur_frm.fields_dict.details.grid;
    ...
    await saveForm(page);
    const clearanceNamePo = await page.evaluate(
      () => window.cur_frm?.doc?.name || ""
    );
    await page.goto(
      `${BASE}/app/pm-clearance/${encodeURIComponent(clearanceNamePo)}`,
      {
        waitUntil: "domcontentloaded",
      }
    );
    ...
    const pass =
      results.every((r) => r.pass !== false) &&
      (await ensureIntroNoConsoleErrors(benign));
    ...
    console.log(
      JSON.stringify({ pass: false, results, evidence, prep }, null, 2)
    );
    await browser.close();
    process.exit(1);
  }
