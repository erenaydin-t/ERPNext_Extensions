...
function bench(expr) {
	const out = execSync(`cd ${BENCH} && bench --site development.localhost execute "${expr}"`, {
		encoding: "utf8",
	});
...
async function run() {
	...
	page.on("request", async (req) => {
		if (!/\/api\/method\/frappe\.desk\.search\.search_link/.test(req.url())) return;
		const post = req.postData() || "";
		requests.push({ phase: "request", url: req.url(), method: req.method(), post });
	});
	page.on("response", async (res) => {
		if (!/\/api\/method\/frappe\.desk\.search\.search_link/.test(res.url())) return;
		let bodyText = "";
		try {
			bodyText = await res.text();
		} catch (_e) {}
		requests.push({ phase: "response", url: res.url(), status: res.status(), bodyText });
	});
	...
	await page.waitForFunction(() => window.cur_frm?.doc?.doctype === "PM Clearance" && !window.cur_frm.is_loading, {
		timeout: 180000,
	});
	...
	await page.locator('[data-fieldname="details"] .grid-add-row').first().click();
	...
	const uiState = await page.evaluate(() => {
		...
		return {
			editableGridMeta: window.cur_frm?.fields_dict?.details?.grid?.meta?.editable_grid,
	...
	const selected = await page.evaluate(() => {
		...
		if (!opt) return { ok: false, reason: "no option", childCount: ul.children.length };
		...
		return {
			...
			ulSameAsVisible: !!document.querySelector(".awesomplete_list_5 ul")?.isSameNode(ul),
		};
	});
	await page.waitForTimeout(1500);
		...
		JSON.stringify(
			{
				...
				selected,
				consoleErrors,
				consoleAll: consoleAll.filter((l) => /PM Clearance|search_link|awesomplete/i.test(l)),
				requests,
				uiState,
				screenshots: [
