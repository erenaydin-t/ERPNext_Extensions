...
# License: MIT
"""PDC / Cheque / Ledger DECIMAL(30,9) allowlist + migrate-safe helpers (v2).

Covers Post Dated Cheque, PDC Allocation, PDC Journal Reference,
Guarantee Document, and Payment Ledger Entry.

Does not widen Facility, Journal Entry, GL Entry, Payment Entry, or Stock fields.
Does not change PDC workflow or accounting behavior.
"""
...
PDC_CHEQUE_LEDGER_FIELDS_BY_DOCTYPE: dict[str, tuple[str, ...]] = {
	"Post Dated Cheque": (
		"cheque_amount",
		"allocated_amount",
		"unallocated_amount",
...
class PdcChequeLedgerFieldTarget:
	doctype: str
	fieldname: str
...
def pdc_cheque_ledger_field_targets() -> tuple[PdcChequeLedgerFieldTarget, ...]:
	return tuple(
		PdcChequeLedgerFieldTarget(doctype=doctype, fieldname=fieldname)
		for doctype, fieldnames in PDC_CHEQUE_LEDGER_FIELDS_BY_DOCTYPE.items()
...
def verify_and_set_metadata(logger) -> list[dict[str, Any]]:
	...
	for target in pdc_cheque_ledger_field_targets():
		row = {
	...
	logger.info(
		"PDC cheque/ledger metadata summary: total=%s changed=%s skipped=%s errors=%s actions=%s",
		summary["total"],
		len(summary["changed"]),
		len(summary["skipped"]),
		len(summary["errors"]),
...
def apply_decimal_schema_targets(logger) -> list[dict[str, Any]]:
	...
	logger.info(
		"PDC cheque/ledger schema summary: total=%s changed=%s skipped=%s errors=%s actions=%s",
		summary["total"],
		len(summary["changed"]),
		len(summary["skipped"]),
		len(summary["errors"]),
