...
async function applyIdEqualsFilter(page, value) {
  await page.evaluate(async (val) => {
    ...
    await lv.filter_area.filter_list.add_filters([
      ["Post Dated Cheque", "name", "=", val],
    ]);
  ...
  try {
    ...
    log(
      ...
      beforeA.data_length > 0 &&
        ...
        !beforeA.get_args.some(
          (f) =>
            f[1] === "name" && f[2] === "=" && (f[3] === "" || f[3] == null)
        ),
    ...
    log(
      ...
      dbgC.data_length === 0 &&
        (await filterXActive(page)) &&
        dbgC.get_args.some((f) => f[3] === "PDC-DOES-NOT-EXIST"),
    ...
    log(
      ...
      dbgD.data_length > 0 &&
        !(await popoverHasEmptyIdEquals(page)) &&
        !dbgD.get_args.some((f) => f[1] === "name" && f[3] === ""),
