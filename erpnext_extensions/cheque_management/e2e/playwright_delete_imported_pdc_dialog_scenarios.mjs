...
const SCREEN = path.join(
  __dirname,
  "screenshots",
  "delete_imported_pdc_dialog_audit"
);
const BASE =
...
async function main() {
  ...
  await page.goto(
    `${BASE}/app/cheque-opening-import/${encodeURIComponent(safe.coi_name)}`,
    {
      waitUntil: "load",
      timeout: 180000,
    }
  ...
  await page.screenshot({
    path: path.join(SCREEN, "Scenario_A_safe_delete_dialog.png"),
    fullPage: true,
  });
  await page
    .locator(".modal-dialog .btn-secondary, .modal-dialog .btn-default")
    .first()
    .click()
  ...
  await page.goto(
    `${BASE}/app/cheque-opening-import/${encodeURIComponent(blocked.coi_name)}`,
    {
      waitUntil: "load",
      timeout: 180000,
    }
