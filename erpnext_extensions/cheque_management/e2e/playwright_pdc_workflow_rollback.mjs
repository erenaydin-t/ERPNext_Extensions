...
async function standardFrappeCancelButtonVisible(page) {
	return page.evaluate(() => {
		const isCancel = (text) => {
			const t = (text || "").trim();
			return t === "Cancel" || (typeof __ !== "undefined" && t === __("Cancel"));
		};
...
async function rollbackViaUi(page, pdcName, targetState, reason, { confirm = true } = {}) {
	if (!(await waitForRollbackButton(page))) {
	...
	const out = await page.evaluate(
		async ({ findBtnSrc, targetState, reason, confirm }) => {
			...
			const preview = (d.fields_dict.preview_html?.$wrapper?.html() || "").trim();
			...
			const reasonFromDialog = (d.get_value("rollback_reason") || reason || "").trim();
			if (!reasonFromDialog) {
				return { ok: false, step: "no_reason", workflow_before: workflowBefore };
			}
			let apiResult;
			try {
				...
				const workflowAfterApiMessage = msg.workflow_state || msg.to_workflow_state || null;
				let workflowAfterReload = null;
				try {
					await cur_frm.reload_doc();
					workflowAfterReload = cur_frm.doc?.workflow_state ?? null;
				} catch (reloadErr) {
					workflowAfterReload = `reload_error:${reloadErr.message || reloadErr}`;
				}
				apiResult = {
					ok: workflowAfterApiMessage === targetFromDialog,
					step: workflowAfterApiMessage === targetFromDialog ? "api_ok" : "api_state_mismatch",
					api_response: msg,
					target_state: targetFromDialog,
	...
	if (!out.ok) {
		return { ...out, pdc_name: pdcName, db_before: stateBeforeDb, db_after: pdcStateFromDb(pdcName) };
	}
...
async function run() {
	...
	try {
		...
		evidence.screenshots.D = await shot(page, "D_issued_workflow_without_cancel_actions");
		results.push({
		...
		results.push({
			...
			ok:
				...
				cancelBlock.before?.workflow_state === cancelBlock.after?.workflow_state,
			cancelBlock,
		});
		...
		const a = await rollbackViaUi(page, prep.payable_registered, "Draft", "E2E rollback A");
		...
		const aDebug = {
			...
			workflow_state_db_immediately_after_api: a.db_immediately_after_api?.workflow_state,
		...
		const b = await rollbackViaUi(page, prep.payable_issued, "Registered", "E2E rollback B");
		...
		results.push({ test: "E_returned_to_issued", ok: e.ok && e.db_after?.workflow_state === "Issued", e });
		...
		results.push({
			...
			ok:
				...
				!fStdCancel &&
				jsErrors.length === 0,
			f: { ...fDoc, has_button: fHasBtn, ui_actions: fUiActions, js_errors: jsErrors, std_cancel: fStdCancel },
		});

		await openPdc(page, prep.payable_cleared_double);
		const j1 = await rollbackViaUi(page, prep.payable_cleared_double, "Issued", "E2E rollback J1");
		...
		const h = await rollbackViaUi(page, prep.payable_cleared_preview, "Issued", "E2E preview H", {
			confirm: false,
		});
		...
		const c1 = await rollbackViaUi(page, prep.payable_cleared, "Issued", "E2E rollback C1");
		evidence.sql.C1 = sqlVerify(prep.payable_cleared);
		evidence.screenshots.C1 = await shot(page, "C_cleared_to_issued");
		const c2 = await rollbackViaUi(page, prep.payable_cleared, "Registered", "E2E rollback C2");
		evidence.sql.C2 = sqlVerify(prep.payable_cleared);
		evidence.screenshots.C2 = await shot(page, "C_issued_to_registered");
		const c3 = await rollbackViaUi(page, prep.payable_cleared, "Draft", "E2E rollback C3");
		...
		await pageG.goto(`${BASE}/app/post-dated-cheque/${encodeURIComponent(prep.receivable_cleared)}`, {
			waitUntil: "load",
			timeout: 180000,
		});
	...
	const sqlSummary = Object.fromEntries(
		Object.entries(evidence.sql).map(([k, v]) => [k, { clean: v?.clean, pdc_name: v?.pdc_name }])
	);
	console.log(
		JSON.stringify({ all_ok, results, evidence: { ...evidence, sql: sqlSummary } }, null, 2)
	);
