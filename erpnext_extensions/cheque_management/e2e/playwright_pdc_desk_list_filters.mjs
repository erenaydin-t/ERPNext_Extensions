...
async function openFilterPopover(page) {
	...
	await page.waitForSelector(".filter-popover", { state: "visible", timeout: 30000 });
...
async function popoverState(page) {
	return page.evaluate(() => {
		...
		const badNameEmpty = boxes.some((box) => {
			...
			const valInput = inputs.find((i) => i.type !== "hidden" && !i.classList.contains("condition"));
...
async function dumpSources(page) {
	return page.evaluate(() =>
		window.erpnext_extensions?.cheque_management?.pdc_list_view?.dump_all_sources?.(window.cur_list)
	);
}
...
async function run() {
	...
	log("seed_db_dirty", dbRows.some((r) => r.user === USER && (r.List?.filters || []).some((f) => f[1] === "name" && f[3] === "")), {
		dbRows,
	});
	...
	try {
		await login(page);
		await page.goto(DESK_LIST, { waitUntil: "domcontentloaded", timeout: 120000 });
		await page.waitForFunction(() => window.cur_list?.doctype === "Post Dated Cheque", {
			timeout: 180000,
		});
