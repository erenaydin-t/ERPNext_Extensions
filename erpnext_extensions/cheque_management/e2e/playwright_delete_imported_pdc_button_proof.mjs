...
const SCREEN = path.join(
  __dirname,
  "screenshots",
  "delete_imported_pdc_button_proof"
);
const BASE =
...
async function main() {
  ...
  await page.goto(
    `${BASE}/app/cheque-opening-import/${encodeURIComponent(prep.coi_name)}`,
    {
      waitUntil: "load",
      timeout: 180000,
    }
  ...
  await page.waitForFunction(
    () =>
      window.cur_frm?.doc?.doctype === "Cheque Opening Import" &&
      !window.cur_frm.is_loading,
    { timeout: 180000 }
  );
  await page.waitForTimeout(5000);
  await page.screenshot({
    path: path.join(SCREEN, "01_coi_form_top_actions.png"),
    fullPage: true,
  });

  await page.goto(
    `${BASE}/app/post-dated-cheque/${encodeURIComponent(prep.pdc_name)}`,
    {
      waitUntil: "load",
      timeout: 180000,
    }
  ...
  const pdcBtn = await page.evaluate(() => {
    const btns = Array.from(
      document.querySelectorAll(".custom-actions .btn, .page-actions .btn")
    );
    return btns.some((b) =>
      (b.textContent || "").includes("Delete Imported PDC")
    );
  });
  const coiDb = getDocumentState("Cheque Opening Import", prep.coi_name, [
    "name",
    "docstatus",
  ]);
  const pdcDb = getDocumentState("Post Dated Cheque", prep.pdc_name, [
    "name",
    "workflow_state",
    "docstatus",
  ]);
  await page.screenshot({
    path: path.join(SCREEN, "02_pdc_form_custom_delete_button.png"),
    fullPage: true,
  });

  await browser.close();
  const all_ok = coiDb.exists && pdcDb.exists && pdcBtn;
  console.log(
    JSON.stringify(
      {
        all_ok,
