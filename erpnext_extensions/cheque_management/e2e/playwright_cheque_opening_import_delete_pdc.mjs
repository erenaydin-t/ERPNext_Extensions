...
async function openCoi(page, name) {
  await page.goto(
    `${BASE}/app/cheque-opening-import/${encodeURIComponent(name)}`,
    {
      waitUntil: "load",
      timeout: 180000,
    }
    ...
    const findBtn = () => {
      const btns = Array.from(
        document.querySelectorAll(
          ".custom-actions .btn, .page-actions .btn, .btn"
        )
      );
      return btns.find((b) =>
...
async function fillDialogReasonAndConfirm(page, reason) {
  const modal = page
    ...
    .filter({ hasText: "Delete Imported PDC" })
    .last();
  await modal.waitFor({ state: "visible", timeout: 60000 });
  const textarea = modal
    .locator('textarea[data-fieldname="reason"], textarea')
    .first();
...
async function scenarioA(page, prep) {
  ...
  const cleared = await page.evaluate((pdc) => {
    ...
    return !items.some(
      (r) => r.imported_pdc === pdc || r.post_dated_cheque === pdc
    );
  }, prep.pdc_name);
  results.A = {
    ok:
...
async function scenarioB(page, prep) {
  ...
  const state = await page.evaluate(() => {
    ...
    return {
      hasBlockers:
        text.includes("Blockers") || text.includes("Journal Reference"),
      primaryLabel: (primary?.textContent || "").trim(),
      reasonFieldInDom: !!reason,
    };
  });
