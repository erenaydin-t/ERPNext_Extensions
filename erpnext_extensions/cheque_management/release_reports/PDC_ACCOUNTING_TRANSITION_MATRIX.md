...
| R2b | Sent to Bank | Registered | JE | Return from Bank | Sent to Bank→Registered | `cheques_in_hand` / `account_paid_to`, `cheques_in_clearing` | CIH / `account_paid_to` | clearing | fallback Return from Bank remark | Yes | Cancel return JE → Sent to Bank | Blocked | Yes; independent JE (no Send JE required); open-occurrence cycles |
