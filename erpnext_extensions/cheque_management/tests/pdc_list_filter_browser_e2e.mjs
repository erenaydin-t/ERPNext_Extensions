...
const BASE_URL =
  process.env.PDC_E2E_BASE_URL || "http://development.localhost:8000";
const USER = process.env.PDC_E2E_USER || "Administrator";
const PASSWORD =
  process.env.PDC_E2E_PASSWORD || process.env.FRAPPE_ADMIN_PASSWORD || "admin";
...
async function login(page) {
  ...
  const email = page
    .locator('input[data-fieldname="email"], #login_email')
    .first();
  const pwd = page
    .locator('input[data-fieldname="password"], #login_password')
    .first();
...
async function evalDebug(page) {
  return page.evaluate(() => {
    ...
    const dbg =
      window.erpnext_extensions?.cheque_management?.pdc_list_view?.get_filter_debug?.(
        lv
      );
    const args = lv?.get_filters_for_args?.() || [];
    const combined = lv?.filter_area?.get?.() || [];
    const xVisible =
      document.querySelector(".filter-x-button")?.offsetParent !== null;
...
async function openFilterPopover(page) {
  ...
  await page.waitForSelector(
    ".filter-popover, .filter-area .filter-edit-area",
    {
      timeout: 15000,
    }
  );
...
async function popoverHasEmptyIdEquals(page) {
  return page.evaluate(() => {
    const boxes = document.querySelectorAll(
      ".filter-popover .filter-box, .filter-area .filter-box"
    );
    for (const box of boxes) {
      const cond = box.querySelector(".condition")?.value || "";
      const fieldLabel =
        box.querySelector(".fieldname .awesomplete input")?.value || "";
      const valInput = box.querySelector(
        ".form-group input.input-with-feedback"
      );
...
async function applyFiltersInPopover(page) {
  const btn = page
    .locator(".filter-popover .apply-filters, .filter-area .apply-filters")
    .first();
...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(() => window.frappe?.boot?.sitename, {
      timeout: 120000,
    });
    await page.evaluate(
      () =>
    ...
    await page.screenshot({
      path: path.join(OUT_DIR, "A_after_load.png"),
      fullPage: true,
    });
    const badNameFilter = (f) =>
      f[1] === "name" &&
      f[2] === "=" &&
      (f[3] === "" || f[3] === null || f[3] === undefined);
    record("A_rows_visible", debugA.rows > 0, {
      rows: debugA.rows,
      combined: debugA.combined,
    });
    record("A_no_name_equals_empty_in_args", !debugA.args.some(badNameFilter), {
      args: debugA.args,
    });
    record("A_filter_button_not_active", !debugA.filterActive, {
      filterActive: debugA.filterActive,
    });
    await openFilterPopover(page);
    const emptyIdA = await popoverHasEmptyIdEquals(page);
    await page.screenshot({
      path: path.join(OUT_DIR, "A_filter_popover.png"),
      fullPage: true,
    });
    ...
    if (!sampleName) {
      ...
      await page
        .locator(".filter-x-button")
        .click({ force: true })
        .catch(() => {});
      await waitForListView(page);
      await openFilterPopover(page);
      await page
        .locator(".filter-popover .add-filter, .filter-area .add-filter")
        .first()
        .click();
      ...
      await page.evaluate((pdcName) => {
        ...
        return fl
          .add_filter("Post Dated Cheque", "name", "=", pdcName)
          .then(() => fl.apply());
      }, sampleName);
      await waitForListView(page);
      const debugB = await evalDebug(page);
      await page.screenshot({
        path: path.join(OUT_DIR, "B_valid_id_filter.png"),
        fullPage: true,
      });
      record(
        "B_filter_preserved",
      ...
      record("B_rows_match", debugB.rows >= 1 && debugB.rows <= 5, {
        rows: debugB.rows,
      });
      record("B_filter_active", debugB.filterActive, {
        filterActive: debugB.filterActive,
      });
    }
    ...
    await page.evaluate(() => {
      const fl = cur_list.filter_area.filter_list;
      return fl
        .add_filter("Post Dated Cheque", "name", "=", "PDC-DOES-NOT-EXIST")
        .then(() => fl.apply());
    });
    await waitForListView(page);
    const debugC = await evalDebug(page);
    await page.screenshot({
      path: path.join(OUT_DIR, "C_intentional_empty.png"),
      fullPage: true,
    });
    record("C_list_empty", debugC.rows === 0, { rows: debugC.rows });
    record("C_filter_still_active", debugC.filterActive, {
      filterActive: debugC.filterActive,
    });
    record(
      "C_filter_preserved",
    ...
    await page.evaluate(() => {
      const fl = cur_list.filter_area.filter_list;
      return fl
        .add_filter("Post Dated Cheque", "name", "=", "")
        .then(() => fl.apply());
    });
    await waitForListView(page);
    const debugD = await evalDebug(page);
    await page.screenshot({
      path: path.join(OUT_DIR, "D_empty_equals_apply.png"),
      fullPage: true,
    });
    record(
    ...
    record("D_rows_visible_after_empty_apply", debugD.rows > 0, {
      rows: debugD.rows,
    });
    ...
    await page.screenshot({
      path: path.join(OUT_DIR, "E_after_reload.png"),
      fullPage: true,
    });
    record("E_rows_after_reload", debugE.rows > 0, { rows: debugE.rows });
    record("E_no_empty_name_filter", !debugE.args.some(badNameFilter), {
      args: debugE.args,
    });
    record("E_filter_inactive", !debugE.filterActive, {
      filterActive: debugE.filterActive,
    });
