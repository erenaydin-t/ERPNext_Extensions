...
function bench(method, argsJson) {
  return argsJson
    ? benchExecute(method, null, JSON.parse(argsJson))
    : benchExecute(method);
}
...
async function run() {
  ...
  try {
    ...
    await page.goto(`${BASE}/desk/facility-type/new-facility-type-1`, {
      waitUntil: "domcontentloaded",
    });
    await page.waitForFunction(
      () => cur_frm?.doc?.doctype === "Facility Type",
      { timeout: 180000 }
    );
    ...
    evidence.screenshots.facility_type = await shot(
      page,
      "A_facility_type_saved"
    );
    results.push({ test: "A_facility_type_master", ok: !!typeName });

    await page.goto(
      `${BASE}/desk/facility/${encodeURIComponent(prep.facility)}`,
      {
        waitUntil: "domcontentloaded",
      }
    );
    await page.waitForFunction(() => cur_frm?.doc?.doctype === "Facility", {
      timeout: 180000,
    });
    ...
    evidence.screenshots.facility_form = await shot(
      page,
      "B_facility_with_type"
    );
    results.push({
      test: "B_facility_form_type_and_name",
      ok:
        facCheck.type === prep.facility_type &&
        facCheck.name === prep.facility_name,
      facCheck,
    });
    ...
    const receiptText = (receiptPrev.rows || [])
      .map((x) => x.user_remark)
      .join(" ");
    results.push({
      test: "C_receipt_preview_facility_name",
      ok:
        receiptText.includes(prep.facility_name) &&
        !/FAC-\d{4}-\d+/.test(receiptText),
      receiptText,
    });
    ...
    await page.waitForFunction(
      () => cur_frm?.doc?.doctype === "Facility Repayment",
      { timeout: 180000 }
    );
    ...
    const repayText = (repayPrev.rows || [])
      .map((x) => x.user_remark)
      .join(" ");
    results.push({
    ...
    const reportCheck = await page.evaluate(
      ...
      },
      {
        ...
        ft: prep.facility_type,
        fname: prep.facility_name,
      }
    );
    results.push({
