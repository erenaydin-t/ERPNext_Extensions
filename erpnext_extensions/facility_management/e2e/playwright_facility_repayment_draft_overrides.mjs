  ...
  await page.waitForFunction(
    (fields) => {
      ...
      for (const sec of ["section_accounts", "section_dimensions"]) {
        const head =
          frm.fields_dict?.[sec]?.$wrapper?.[0]?.querySelector?.(
            ".section-head"
          );
 ...
 */
async function waitAndAuditRepaymentFields(
  page,
  fieldnames,
  { docstatusExpected, expectEditable }
) {
    ...
    async ({
      ...
      const readFieldState = (fn) => {
        ...
        const controlDisabled =
          fld?.disable === true || fld?.read_only === true;
...
async function applyOverridesViaApi(page, repayment, overrides) {
  ...
  await page.goto(
    `${BASE}/desk/facility-repayment/${encodeURIComponent(repayment)}`,
    {
...
async function run() {
  ...
  try {
    await login(page);
    await page.goto(
      `${BASE}/desk/facility-repayment/${encodeURIComponent(prep.repayment)}`,
      {
        waitUntil: "domcontentloaded",
      }
    );
    ...
    await page.screenshot({
      path: path.join(SCREEN, "01_draft_overrides.png"),
      fullPage: true,
    });
    ...
    const previewAccounts = new Set(
      (preview.rows || []).map((row) => row.account)
    );
    results.push({
    ...
    await page.screenshot({
      path: path.join(SCREEN, "02_preview.png"),
      fullPage: true,
    });
    ...
    const dbJe = await waitDocstatus(
      "Journal Entry",
    ...
    const dbRepay = await waitDocstatus(
      "Facility Repayment",
      prep.repayment,
      1,
      {
    ...
    results.push({
      ...
      jeAccounts,
      db: {
        je: getDocumentState("Journal Entry", submitted.journal_entry, [
          "name",
          "docstatus",
        ]),
        repayment: getDocumentState("Facility Repayment", prep.repayment, [
          "name",
          "docstatus",
        ]),
      },
    ...
    await page.goto(
      `${BASE}/desk/journal-entry/${encodeURIComponent(
        submitted.journal_entry
      )}`,
      {
        waitUntil: "domcontentloaded",
    ...
    await page.screenshot({
      path: path.join(SCREEN, "03_submitted_je.png"),
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: path.join(SCREEN, "04_submitted_read_only.png"),
      fullPage: true,
    });
  } finally {
    await browser.close();
  ...
  console.log(
    JSON.stringify(
      { all_ok, results, screenshots: SCREEN, prep: prep.repayment },
      null,
      2
    )
  );
