...
async function investigationDump(page) {
	return page.evaluate(() => {
		...
		return {
			has_defaults_js:
				typeof erpnext_extensions?.facility_management?.defaults?.apply_from_company === "function",
			handlers: Object.keys(handlers),
			meta_js_has_defaults: String(frappe.get_meta("Facility").__js || "").includes(
				"facility_settings_defaults"
			),
			meta_js_has_dimension: String(frappe.get_meta("Facility").__js || "").includes(
				"facility_dimension_link_queries"
			),
...
async function run() {
	const prep = benchExecute("erpnext_extensions.facility_management.e2e.facility_dimension_link_prep.prepare");

	const browser = await chromium.launch({ headless: true });
	const context = await browser.newContext({ locale: "en-US", viewport: { width: 1500, height: 950 } });
	...
	page.on("response", async (response) => {
		...
		if (url.includes("get_facility_settings_defaults") || url.includes("method")) {
			try {
	...
	try {
		...
		log("js_loaded_and_handlers_registered", invBefore.has_defaults_js && invBefore.handlers.includes("company"), {
			invBefore,
		});

		if (!invBefore.has_defaults_js) {
			throw new Error("facility_settings_defaults.js not loaded — aborting (no fallback)");
		}
		...
		const accountsOk = ACCOUNT_FIELDS.every(
			(fn) => !!afterCompany[fn] && afterCompany[fn] === apiBefore.defaults?.[fn]
		);
		const dimsOk = DIMENSION_FIELDS.every((fn) => {
			const exp = apiBefore.defaults?.[fn];
			return !exp || afterCompany[fn] === exp;
		});
		log("A_new_facility_defaults_from_desk", apiBefore.found && accountsOk && dimsOk, {
			afterCompany,
		...
		const overrideVal = await page.evaluate(async () => {
			const r = await frappe.call({
				...
				args: {
					...
					txt: "",
					page_length: 15,
					filters: { company: cur_frm.doc.company, account_type: "Bank", is_group: 0 },
				},
			});
			const alt = (r.message || [])
				.map((x) => x.value)
		...
		if (saveResult.ok) {
			...
			await page.goto(`${BASE}/desk/facility/${encodeURIComponent(saveResult.name)}`, {
		...
		if (noFs.company) {
			...
			const stillUsable = await pageD.evaluate(() => !!(window.cur_frm && cur_frm.is_new()));
			log("D_missing_facility_settings", stillUsable && !missingDoc.bank_account, {
			...
			log("D_missing_facility_settings", false, { error: "could not resolve company without settings" });
		}
		...
		log("E_api_server_fallback", !!apiInsert.bank_account && !!apiInsert.name, { apiInsert });
	} finally {
		await browser.close();
	}
