...
function assertReceiptRows(prep) {
  const {
    ...
    loan_gl,
    deferred_gl,
    expected_bank_dimension,
    facility,
  } = prep;
  const bank = je_rows.find((r) => r.account === bank_gl && r.debit > 0);
  const deferred = je_rows.find(
    (r) => r.account === deferred_gl && r.debit > 0
  );
  const loanCredits = je_rows.filter(
    (r) => r.account === loan_gl && r.credit > 0
  );
  const errors = [];
  if (!bank?.dims?.bank_dimension)
    errors.push("bank row missing bank_dimension");
  if (bank?.dims?.bank_dimension !== expected_bank_dimension)
    errors.push("bank_dimension mismatch");
  if (bank?.dims?.department) errors.push("bank row has department");
  if (bank?.dims?.bank_account_dimension)
    errors.push("bank row has bank_account_dimension");
  if (dim_fn && bank?.dims?.[dim_fn])
    errors.push("bank row has facility dimension");

  if (!deferred) errors.push("deferred row missing");
  if (loanCredits.length !== 2)
    errors.push(`expected 2 loan credit rows, got ${loanCredits.length}`);
  ...
  for (const [label, row] of [
    ...
    if (dim_fn && row.dims?.[dim_fn] !== facility)
      errors.push(`${label} facility dim`);
    if (row.dims?.department) errors.push(`${label} has department`);
    if (row.dims?.bank_dimension) errors.push(`${label} has bank_dimension`);
    if (row.dims?.bank_account_dimension)
      errors.push(`${label} has bank_account_dimension`);
  }
...
async function run() {
  ...
  try {
    ...
    const uiRows = await page.evaluate(() => {
      const dimFn = frappe.boot?.accounting_dimensions?.find?.(
        (d) => d.document_type === "Facility"
      )?.fieldname;
