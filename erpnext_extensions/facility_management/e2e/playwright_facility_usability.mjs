...
async function run() {
	...
	const evidence = { prep, screenshots: {}, repayment: null, preview: null, submittedJe: null, reports: {} };
	const results = [];
	...
	async function openNewRepaymentForm() {
		await page.goto(`${BASE}/desk/facility-repayment/new-facility-repayment-1`, {
	...
	try {
		...
		const visible = await page.evaluate(() => !!cur_frm.fields_dict?.interest_expense_account);
		evidence.screenshots.interest_field = await shot(page, "01_interest_expense_field_visible");
		...
		evidence.screenshots.after_facility_select = await shot(page, "02_interest_autofill");
		results.push({
			test: "2_interest_autofill",
			ok: !!afterSelect.interest && afterSelect.facility_name === prep.facility_name,
			afterSelect,
		});
		...
		const overrideAccount = await page.evaluate(async () => {
			const r = await frappe.call({
				...
				args: {
					...
					txt: "",
					page_length: 20,
					filters: { company: cur_frm.doc.company, root_type: "Expense", is_group: 0 },
				},
			});
		...
		evidence.screenshots.after_override = await shot(page, "03_interest_override");
		...
		evidence.screenshots.preview_dialog = await shot(page, "04_after_preview_call");
		results.push({
		...
		const jeEvidence = await page.evaluate(async ({ je, expectedInterest }) => {
			...
			const intRows = doc.accounts.filter(
				(a) => a.debit_in_account_currency > 0 && a.account === expectedInterest
			);
			return {
				voucher_type: doc.voucher_type,
				row_count: doc.accounts.length,
				interest_rows: intRows.map((a) => ({ account: a.account, debit: a.debit_in_account_currency })),
			};
		...
		await page.goto(`${BASE}/desk/facility-repayment`, { waitUntil: "domcontentloaded" });
		await page.waitForTimeout(2000);
		evidence.screenshots.repayment_list = await shot(page, "06_facility_repayment_list");
		const listHas = listSearch.some((row) => row[2] === prep.facility || row[1] === prep.facility_name);
		results.push({
		...
		results.push({
			...
			ok: reports.balance.count >= 1 && reports.balance.facilities.includes(prep.facility),
			balance: reports.balance,
		});

		await page.goto(`${BASE}/desk/query-report/Facility%20Balance`, { waitUntil: "domcontentloaded" });
		await page.waitForTimeout(2500);
		evidence.screenshots.facility_balance = await shot(page, "07_facility_balance_report");

		results.push({
			test: "9_facility_ledger_facility_name_filter",
			ok: reports.ledger.count >= 1 && reports.ledger.sample?.facility === prep.facility,
			ledger: reports.ledger,
		});
		await page.goto(`${BASE}/desk/query-report/Facility%20Ledger`, { waitUntil: "domcontentloaded" });
		await page.waitForTimeout(2500);
		evidence.screenshots.facility_ledger = await shot(page, "08_facility_ledger_report");
