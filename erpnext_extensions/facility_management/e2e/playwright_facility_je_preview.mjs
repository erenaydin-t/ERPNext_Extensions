...
function rowsMatch(previewRows, jeAccounts) {
	...
	for (let i = 0; i < previewRows.length; i++) {
		...
		if (Number(p.debit || 0) !== Number(j.debit_in_account_currency || 0)) return false;
		if (Number(p.credit || 0) !== Number(j.credit_in_account_currency || 0)) return false;
	}
	return true;
}
...
async function run() {
	...
	try {
		...
		await page.goto(`${BASE}/desk/facility/${encodeURIComponent(receiptPrep.facility)}`, {
			waitUntil: "domcontentloaded",
		});
		await page.waitForFunction(
			() => window.cur_frm?.doc?.doctype === "Facility" && !window.cur_frm.is_loading,
			{ timeout: 180000 }
		);
		...
		const createReceipt = await page.evaluate(async () => {
			const before = await frappe.db.get_value("Facility", cur_frm.doc.name, "receipt_journal_entry");
			const r = await frappe.call({
				method:
		...
		await page.goto(
			`${BASE}/desk/facility-repayment/${encodeURIComponent(repayPrep.repayment)}`,
			{ waitUntil: "domcontentloaded" }
		);
		await page.waitForFunction(
			() => window.cur_frm?.doc?.doctype === "Facility Repayment" && !window.cur_frm.is_loading,
			{ timeout: 180000 }
		);
		...
		await page.waitForSelector(".modal-dialog:visible", { timeout: 30000 });
		evidence.screenshots.repayment_preview = await shot(page, "C_repayment_preview_dialog");
		results.push({
		...
		const submitRepay = await page.evaluate(async (previewRows) => {
			...
			return { repayment: doc.name, je: doc.journal_entry, accounts: jeDoc.accounts, previewRows };
		}, repayPreviewUi.rows);
