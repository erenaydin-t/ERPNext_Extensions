...
async function linkDropdownHasResults(page, fieldname) {
	...
	const probe = String(samples[0].value || samples[0].description || "a").slice(0, 3);
	const control = page.locator(`.frappe-control[data-fieldname="${fieldname}"]`).first();
...
async function run() {
	...
	try {
		...
		await page.goto(`${BASE}/desk/facility-settings/${encodeURIComponent(prep.facility_settings_name)}`, {
		...
		log(
			...
			fsFields.every((fn) => fsDropdowns[fn].items > 0 && fsDropdowns[fn].api_count > 0),
			{ fsDropdowns, screenshot: await screenshot(page, "02_facility_settings_dropdowns") }
		);
		...
		await page.locator('.btn-primary[data-label="Save"], button[data-label="Save"]').first().click();
		await page.waitForTimeout(2500);
		log("facility_settings_saved", true, { fsPicked, screenshot: await screenshot(page, "03_facility_settings_saved") });

		await page.goto(`${BASE}/desk/facility/new-facility-1`, { waitUntil: "domcontentloaded" });
		await waitFormReady(page);
		...
		const facFields = ["department", "bank_dimension", "bank_account_dimension"];
		const facDropdowns = {};
		...
		log(
			...
			facFields.every((fn) => facDropdowns[fn].items > 0 && facDropdowns[fn].api_count > 0),
			{ facDropdowns, screenshot: await screenshot(page, "04_facility_new_dropdowns") }
		);
		...
		log(
			...
			{ savedName, facPicked, screenshot: await screenshot(page, "05_facility_saved") }
		);
