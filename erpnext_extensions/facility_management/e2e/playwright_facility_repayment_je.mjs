...
async function run() {
  ...
  try {
    await login(page);
    await page.goto(
      `${BASE}/desk/facility-repayment/${encodeURIComponent(draft.repayment)}`,
      {
        waitUntil: "domcontentloaded",
      }
    );
    ...
    results.push({
      ...
      ok:
        uiPreview.balanced &&
        uiPreview.rows.length === 6 &&
        uiPreview.total_debit === 1140,
      uiPreview,
    });
    ...
    const dbJe = await waitDocstatus("Journal Entry", saveSubmit.je, 1, {
      timeoutMs: 120000,
    });
    ...
    results.push({
      ...
      saveSubmit,
      jeRows,
      db_je: getDocumentState("Journal Entry", saveSubmit.je, [
        "name",
        "docstatus",
      ]),
      db_wait: dbJe,
  ...
  console.log(
    JSON.stringify({ all_ok, results, serverPreview: preview, draft }, null, 2)
  );
