...
# License: MIT
"""Petty Management DECIMAL(30,9) allowlist + migrate-safe helpers (v2).

Covers Petty Management parents and their child tables with an explicit static
allowlist taken from DocType JSON metadata:

- PM Holder
- PM Request / PM Request Detail
- PM Clearance / PM Clearance Detail / PM Clearance Request Allocation
- PM Opening Advance

PM Settings has no Currency/Float monetary columns and is excluded.
Does not scan DocType fields at runtime or widen unrelated DocTypes.
"""
...
# Built from petty_management DocType JSON Currency fields (no Float monetary fields found).
PETTY_MANAGEMENT_FIELDS_BY_DOCTYPE: dict[str, tuple[str, ...]] = {
	"PM Holder": (
		"max_balance",
		"account_gl_balance",
		"current_balance",
	...
	"PM Request Detail": ("advance_amount",),
	"PM Clearance": (
		"funded_available",
		"opening_available",
		"total_available",
...
class PettyManagementFieldTarget:
	doctype: str
	fieldname: str
...
def petty_management_field_targets() -> tuple[PettyManagementFieldTarget, ...]:
	return tuple(
		PettyManagementFieldTarget(doctype=doctype, fieldname=fieldname)
		for doctype, fieldnames in PETTY_MANAGEMENT_FIELDS_BY_DOCTYPE.items()
...
def verify_and_set_metadata(logger) -> list[dict[str, Any]]:
	...
	for target in petty_management_field_targets():
		row = {
	...
	logger.info(
		"Petty Management metadata summary: total=%s changed=%s skipped=%s errors=%s actions=%s",
		summary["total"],
		len(summary["changed"]),
		len(summary["skipped"]),
		len(summary["errors"]),
...
def apply_decimal_schema_targets(logger) -> list[dict[str, Any]]:
	...
	logger.info(
		"Petty Management schema summary: total=%s changed=%s skipped=%s errors=%s actions=%s",
		summary["total"],
		len(summary["changed"]),
		len(summary["skipped"]),
		len(summary["errors"]),
